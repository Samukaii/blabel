import { Component, computed, DOCUMENT, inject, input, OnInit, output } from "@angular/core";
...
@Component({
    ...
    imports: [ReactiveFormsModule, TranslationsFormLanguageComponent, CallPipe, IconComponent]
})
export class TranslationsFormComponent implements OnInit {
    ...
    formValue = toSignal(this.form.events.pipe(map(() => this.form.getRawValue())), { initialValue: this.form.getRawValue() })
    ...
    fillWithAi() {
        console.log(this.getFormValue().entries);

        this.aiService.fillWithAi(this.getFormValue().entries, this.additionalContextControl.value).subscribe((response: any) => {
